...
const blocked = [
  ...
  { name: 'per-app JWT signing secret', pattern: /\binvesting-jwt-signing-secret\b/ },
  { name: 'jsonwebtoken dep / use', pattern: /\bjsonwebtoken\b/ },
  { name: 'jose dep / use', pattern: /\bfrom\s+['"]jose['"]/ },
  { name: 'jwt.sign / jwt.verify', pattern: /\bjwt\.(sign|verify)\b/ },
  { name: 'createRemoteJWKSet', pattern: /\bcreateRemoteJWKSet\b/ },
  { name: 'createRequireAuth jwtSecret param', pattern: /createRequireAuth\s*\(\s*\{\s*jwtSecret/ },
  { name: 'POST /api/auth/exchange', pattern: /\/api\/auth\/exchange\b/ },
  { name: 'localStorage token', pattern: /localStorage\.(get|set|remove)Item\(['"]token['"]/ },
